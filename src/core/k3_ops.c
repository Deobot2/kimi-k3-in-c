               ...
               int renorm, float routed_scale, float *scratch)
{
     ...
     * uninitialised stack.
     *
     * score/choice used to malloc/free here on every call -- once per token per MoE
     * layer, up to 92 times per generated token -- so they now come from caller-owned
     * scratch (2*n_experts floats) instead. */
    float *score  = scratch;
                 ...
                 int nsrc, int n, float eps, float *score)
{
     ...
     *
     * score used to malloc/free here on every call, up to twice per layer per token
     * across every layer; nsrc is bounded by n_layers/attn_res_block + 2, which callers
     * already size scratch for, so it is now caller-owned instead. */
                              ...
                              const K3Cfg *c, int T, float *scratch)
{
    ...
    for (int t = 0; t < T; t++) {
        ...
        /* Routing for every token in the chunk finishes before step 2 touches gu/act/edn
         * below, so the router's own scratch (the same 2*n_experts floats k3_moe
         * reserves at the front of this same buffer) is free to reuse as scratch here
         * too. */
        k3_router(it, wtt, xt, w->gate, w->bias, E, c->n_experts, K,
                  c->moe_renorm, c->routed_scale, scratch);
     ...
     * that selected it. gu/act/edn are reused per (expert, token), past the router
     * scratch at the front of this buffer, which every token's routing above is now
     * done reading. */
    float *gu  = scratch + 2 * c->n_experts; /* [2*I] */
    float *act = gu + 2 * I;              /* [I]   */
    float *edn = act + I;                 /* [Ll]  */
...
size_t k3_layer_scratch_kv(const K3Cfg *c, int T, int span, int mode)
{
    ...
    size_t sub = a > b ? a : b;
    if (m > sub) sub = m;
    /* prefix_sum, tmp, fold vectors, one attn_res source stack, its score row, plus the
     * sub-block. The score row is k3_attn_res's own scratch (nsrc floats, nsrc bounded
     * by the same maxb as the source stack) rather than a malloc/free per call. */
                         ...
                         const K3KvCache *kv)
{
    ...
    float *ares   = dgu + (size_t)2 * c->dense_inter; /* [maxb] k3_attn_res score */
    float *sub    = ares + (size_t)maxb;

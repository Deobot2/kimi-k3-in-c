                 ...
                 float *u_scratch)
{
    ...
    /* Written AFTER the decay above has already modified S. Returning early here
     * would leave the recurrent state permanently scaled but never updated -- silent,
     * unrecoverable corruption of every subsequent token. u_scratch is caller-owned:
     * this step runs T*H times per layer, once per token per head, and a calloc()/free()
     * pair on that path measured as real overhead for no benefit -- the buffer never
     * needs to survive past this call. */
    float *u = u_scratch;
...
{
    /* scratch is caller-owned, at least nsrc floats: this runs once per token per
     * attn_res block boundary, and a malloc()/free() pair on that path was pure
     * allocator churn, same fix and same reason as k3_kda_step and k3_router above. */
    float *score = scratch;
                  ...
                  int T, float *state, float *scratch)
{
     ...
     * core counts. wr and ur are each a full P-wide row, so wr + h*D and ur + h*D give
     * each head its own private q-scale slice and its own private k3_kda_step scratch,
     * with no new allocation and no allocation at all inside the per-token step. */
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
    for (int h = 0; h < H; h++) {
        float *wh = wr + (size_t)h * D;
        float *uh = ur + (size_t)h * D;
        for (int t = 0; t < T; t++) {
...
size_t k3_layer_scratch_kv(const K3Cfg *c, int T, int span, int mode)
{
    ...
    /* prefix_sum, tmp, fold vectors, one attn_res source stack, k3_attn_res's own score
     * scratch (maxb floats -- one per source row, caller-owned for the same reason
     * k3_kda_step's and k3_router's were), plus the sub-block */
                         ...
                         const K3KvCache *kv)
{
    ...
    float *asc    = dgu + (size_t)2 * c->dense_inter;   /* [maxb] k3_attn_res scratch */
    float *sub    = asc + (size_t)maxb;
    ...
    if (*n_blocks > 0) {
        for (int t = 0; t < T; t++) {
            ...
            memcpy(src + (size_t)(*n_blocks) * E, pref + (size_t)t * E,
                   (size_t)E * sizeof(float));
            k3_attn_res(h + (size_t)t * E, src, foldA, *n_blocks + 1, E, c->rms_eps, asc);
        }
    }
    ...
    for (int t = 0; t < T; t++) {
        ...
        memcpy(src + (size_t)(*n_blocks) * E, pref + (size_t)t * E,
               (size_t)E * sizeof(float));
        k3_attn_res(h + (size_t)t * E, src, foldM, *n_blocks + 1, E, c->rms_eps, asc);
    }

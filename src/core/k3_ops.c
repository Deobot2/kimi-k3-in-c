                 ...
                 float *u)
{
     ...
     * `u` is caller-owned: this step runs once per head per token, so an allocation
     * here would be the hottest heap traffic in the engine. Zeroed here rather than by
     * the caller because it is a true accumulator, not an input. */
                  ...
                  int T, float *state, float *scratch)
{
     ...
     * core counts. wr and wu are full P-wide rows, so wr + h*D and wu + h*D give each
     * head its own private D-wide slice of each -- the pre-scaled query and the
     * recurrence's u accumulator -- with no new allocation, here or in k3_kda_step. */
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
    for (int h = 0; h < H; h++) {
        float *wh = wr + (size_t)h * D;
        float *uh = wu + (size_t)h * D;
        for (int t = 0; t < T; t++) {

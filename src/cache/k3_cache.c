 ...
 */
/* The small and main queues are each a singly-linked list threaded through q_next[],
 * with their own head/tail/len fields. Every operation on "queue q" needs all three,
 * selected the same way, so that selection lives here once. */

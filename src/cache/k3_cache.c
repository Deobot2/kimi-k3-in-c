 ...
 */
/* The small and main queues are each three fields (head/tail/len); every queue op needs
 * pointers to the right ones for the SMALL/MAIN it was asked to touch. One place to
 * resolve them, rather than the same ternary triple copied at each call site, so a third
 * queue would only need to change here. */
static void queue_ptrs(K3Cache *c, int q, int32_t **head, int32_t **tail, int32_t **len)
{

 ...
 */
/* Small and main are the only two queues, each a trio of (head, tail, len) fields on
 * K3Cache itself rather than an array of queue structs, so every function that walks
 * one queue by its K3_Q_SMALL/K3_Q_MAIN tag needs these three pointers resolved the
 * same way. */

                         ...
                         int *seq, float *ks, Weights *w)
{
    ...
    if (w->kv_mode == K3_KV_LATENT) {
        ...
        if (hd->kv_rows > w->kv_cap) {
            fprintf(stderr, "REFUSING: %s holds %d positions of expanded KV, this run's "
                            "cache is %d.\n  Raise --gen or shorten the prompt.\n",
...
"                        sweep: the argmax after positions 0..i compared against the id\n"
"                        the sequence actually continues with. This is the acceptance\n"
"                        rate a draft model would see under batched greedy verification\n"
...
int main(int argc, char **argv)
{
    ...
    if (trace_dir) k3_cache_set_trace(&cache, 1);

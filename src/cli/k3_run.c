...
"  --trunk-gb X          trunk ring / pinned-layer budget (default 16). Only takes\n"
"                        effect with --trunk; a resident (unstreamed) trunk ignores it\n"
...
"  --cache-gb X          routed-expert cache budget (default 64). Always applies,\n"
"                        including with a resident trunk, so it adds to the trunk's\n"
"                        108.81 GB rather than sharing a preset's split with it\n"
"\n"
"generation:\n"
...
"  --tf-check            teacher-forced agreement over the whole --ids sequence in ONE\n"
"                        sweep: prediction i is the argmax after positions 0..i,\n"
"                        compared against the id the sequence actually continues with.\n"
"                        This is the acceptance rate batched greedy verification would\n"
"                        see, which free-running comparisons cannot measure\n"
...
int main(int argc, char **argv)
{
    ...
    printf("%d tokens in %.1f s, %.2f s/token average\n",
           nout, t_total, nout > 0 ? t_total / nout : 0.0);

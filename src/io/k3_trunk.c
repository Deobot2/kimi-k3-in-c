                  ...
                  int ring_want)
{
    ...
    if (!root) {
        ...
        free(txt);
        k3_trunk_close(tr);   /* releases tr->json_arena/tr->json_root, set just above */
        return -1;
    }
     ...
     * which is exactly the synchronous path this file had before the reader existed. */
    {
        ...
        if (RING >= 2) {
            ...
            if (pthread_create(&io->thread, NULL, trunk_io_main, io) != 0) {
                ...
                free(io);
                tr->io_state = NULL;
                k3_trunk_close(tr);   /* releases arena, pins, slot_of/reads_of, etc. */
                return -1;
            }
            io->running = 1;
        }
...
bad:
    free(txt);
    k3_trunk_close(tr);   /* releases json_arena/json_root and any tr->lay[] built so far */
    return -1;
}

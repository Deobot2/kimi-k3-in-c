 ...
 * bind_wall - load_seconds - widen_wall is the remaining unattributed time. They live on
 * the K3Trunk itself (tr->bind_wall etc, declared in k3_trunk.h) rather than as globals,
 * because hybrid decode binds a second, independent K3Trunk for the draft model on every
 * step and a global would let its binds silently inflate the exact model's report. */

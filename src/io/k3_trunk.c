 ...
 *
 * These three counters (K3Trunk::bind_wall/widen_wall/binds) close the gap by
 * measurement rather than estimate: wall clock around the whole of k3_trunk_bind, of
 * which the widen loop is tracked separately, so bind_wall - load_seconds - widen_wall
 * is the remaining unattributed time. */

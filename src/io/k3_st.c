...
int k3_st_open(K3St *s, const char *dir)
{
    ...
    while ((e = readdir(d))) {
        ...
        if (nf == cf) {
            ...
            /* Do not overwrite `files` with the NULL realloc returns on failure: that
             * would both leak every path already collected and hand files[nf] below a
             * null pointer to write through. */
            if (!nfiles) {

...
static int scan_shard(K3St *s, Build *b, int shard, const char *path)
{
    ...
    if (fsize < 0) {
        fprintf(stderr, "k3_st: %s: lseek failed\n", path);
        close(fd); return -1;
    }
...
int k3_st_open(K3St *s, const char *dir)
{
    ...
    const size_t dirlen = strlen(dir);   /* loop-invariant: dir never changes below */
    struct dirent *e;
    while ((e = readdir(d))) {
        ...
        if (nf == cf) {
            cf = cf ? cf * 2 : 32;
            char **grown = (char **)realloc(files, cf * sizeof *files);
            if (!grown) {

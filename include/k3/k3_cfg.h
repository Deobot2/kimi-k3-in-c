...
static inline int k3_cfg_load_file(K3Cfg *c, int *fa, int fa_max, const char *path)
{
    ...
    /* json_parse (third_party/json.h) copies every string into its own allocation as it
     * goes (j_dup) rather than pointing into `text`, so nothing in `root` references
     * `txt` once parsing returns, success or failure. */
    free(txt);

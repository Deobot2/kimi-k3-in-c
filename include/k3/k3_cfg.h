...
/* Convenience: read and parse a config file, then load it. K3Cfg copies out every
 * value it keeps -- full_attn is the one pointer-shaped field, and it points into the
 * caller-owned fa buffer, never into the parsed document -- so nothing needs txt, root
 * or arena to survive this call, and all three are freed before returning. */

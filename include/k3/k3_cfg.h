...
static inline int k3_cfg_load(K3Cfg *c, int *fa, int fa_max, jval *root, const char *whence)
{
    ...
    if (c->kda_head_dim > K3_MAX_KDA_HEAD_DIM) {
        fprintf(stderr, "k3_cfg: %s has kda_head_dim %d, but this build supports at "
                        "most %d\n  (K3_MAX_KDA_HEAD_DIM in k3.h bounds the "
                        "stack-allocated recurrence temporary)\n",
                whence, c->kda_head_dim, K3_MAX_KDA_HEAD_DIM);
        return 0;
    }
...
static inline int k3_cfg_load_file(K3Cfg *c, int *fa, int fa_max, const char *path)
{
    ...
    /* json_parse copies every string it needs via j_dup (see third_party/json.h), so
     * nothing in `root` points into `txt` -- safe to free here on either outcome. */
    free(txt);

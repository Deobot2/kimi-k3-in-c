...
/* Convenience: read and parse a config file, then load it. K3Cfg copies every value it
 * needs out of the tree (full_attn lands in the caller's fa[], everything else is a
 * plain int/float), so nothing in it points back into `root` or `txt` -- both are
 * freed here rather than leaked for the process lifetime. */
